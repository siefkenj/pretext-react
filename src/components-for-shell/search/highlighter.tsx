...
export const Highlighter = ({
    ...
    const highlighterChildren: HastRoot["children"] = [];
    for (const child of parsedHtml.children) {
        if (child.type !== "text") {
            highlighterChildren.push(child);
            continue;
        }
        let remaining = child.value;
        while (remaining) {
            let bounds: MatchBounds | null = null;
            // Find the first possible match.
            const matches = search
                .map((str) => getMatchBounds(remaining, str))
                .filter((x) => x);
            matches.sort((a, b) => Number(+(a?.start || 0) > +(b?.start || 0)));
            bounds = matches[0] || null;

            // No match anywhere in the remaining string, stop.
            if (!bounds) {
                highlighterChildren.push({ type: "text", value: remaining });
                break;
            }
            ...
            if (nonMatch) {
                highlighterChildren.push({ type: "text", value: nonMatch });
            }
    ...
    parsedHtml.children = highlighterChildren;
